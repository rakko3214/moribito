...
export function createGame(parent: HTMLElement, onReady: () => void) {
  ...
  const game = new Phaser.Game({ type: Phaser.AUTO, parent, backgroundColor: "#17251f", scale: { mode: Phaser.Scale.RESIZE, width: "100%", height: "100%" }, render: { antialias: false, pixelArt: true }, scene: [new BootScene(), new WorldScene(bridge)] });
